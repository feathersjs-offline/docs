...
* `src/` contains the Feathers server code.
  ...
  * `channels.ts` sets up Feathers [event channels](https://docs.feathersjs.com/api/channels.html#channels)
...
The most important pattern used in the generated application to split things up into individual files are _configure functions_ which are functions that are exported from a file and take the Feathers [app object](https://docs.feathersjs.com/api/application.html) and then use it to e.g. register services. Those functions are then passed to [app.configure](https://docs.feathersjs.com/api/application.html#configurecallback).
